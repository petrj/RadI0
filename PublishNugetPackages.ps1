cd $PSScriptRoot


function Get-SecureStringFromUserInput
{
    [CmdletBinding()]
    param(
        [Parameter(Mandatory=$false)]
        [string] $Message = 'Enter password:',

        [Parameter(Mandatory=$false)]
        [switch] $AsPlainText
    )

    process {
        Write-Host $Message -NoNewline
        $secureToken = Read-Host -AsSecureString

        $ptr = [Runtime.InteropServices.Marshal]::SecureStringToBSTR($secureToken)
        try {
            $plainToken = [Runtime.InteropServices.Marshal]::PtrToStringBSTR($ptr)
        }
        finally {
            [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($ptr)
        }

        if ($AsPlainText) {
            Write-Output $plainToken
        }
        else {
            Write-Output $secureToken
        }
    }
}

if ([string]::IsNullOrEmpty($env:GITHUB_TOKEN))
{
    $token = Get-SecureStringFromUserInput -AsPlainText -Message "Enter github access token:"
}
else
{
    Write-Host "Using GITHUB_TOKEN from environment variable."
    $token = $env:GITHUB_TOKEN
}

function Publish-Project
{
    [CmdletBinding()]
    param (
        [string]$ProjectName,
        [string]$PackageVersion,
        [string]$Token,
        [string]$PSScriptRoot
    )
    process
    {
        $ProjectPath = Join-Path $PSScriptRoot -ChildPath "$ProjectName\$ProjectName.csproj"

        dotnet build $ProjectPath -c Release /p:PackageVersion=$packageVersion

        $fName = Join-Path $PSScriptRoot -ChildPath "$ProjectName\bin\Release\$ProjectName.$PackageVersion.nupkg"

        dotnet nuget push $fName -k $Token --source "github"  --timeout 3000 # --skip-duplicate
    }
}

Publish-Project -ProjectName "RTLSDR" -PackageVersion "1.0.13" -PSScriptRoot $PSScriptRoot -Token $token
Publish-Project -ProjectName "RTLSDR.Audio" -PackageVersion "1.0.26" -PSScriptRoot $PSScriptRoot -Token $token
Publish-Project -ProjectName "RTLSDR.Common" -PackageVersion "1.0.12" -PSScriptRoot $PSScriptRoot -Token $token
Publish-Project -ProjectName "RTLSDR.FM" -PackageVersion "1.0.9" -PSScriptRoot $PSScriptRoot -Token $token
Publish-Project -ProjectName "RTLSDR.DAB" -PackageVersion "1.0.19" -PSScriptRoot $PSScriptRoot -Token $token



