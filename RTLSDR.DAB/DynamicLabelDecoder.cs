...
using LoggerService;
using RTLSDR.Common;
using RTLSDR.DAB.MOT;

namespace RTLSDR.DAB
{
    /// <summary>
    /// Decodes Programme Associated Data (PAD) from DAB+ AAC Access Units,
    /// including Dynamic Label Segments (DLS) and MOT SlideShow images (SLS).
    ///
    /// References:
    /// - ETSI EN 300 401 §7.4 (PAD structure)
    /// - ETSI EN 300 401 §7.4.5 (Dynamic Label)
    /// - ETSI EN 301 234 (Multimedia Object Transfer - MOT)
    ...
    public class DynamicLabelDecoder
    {
        ...
        // DGLI and MOT decoders
        private readonly DGLIDecoder _dgliDecoder = new DGLIDecoder();
        private readonly MOTDecoder _motDecoder = new MOTDecoder();
        private readonly MOTManager _motManager;
        ...
        /// </summary>
        public event EventHandler<DABSlide>? OnSlideShowChanged;
        ...
        /// <summary>
        /// The most recently decoded DAB slide (image).
        /// </summary>
        public DABSlide? LastSlide => _motManager.LastSlide;

        /// <summary>
        /// The X-PAD Application Type for MOT SlideShow (defaults to 12).
        /// </summary>
        public int MOTAppType { get; set; } = 12;

        public DynamicLabelDecoder(ILoggingService loggingService)
        {
            _loggingService = loggingService;
            _motManager = new MOTManager(loggingService);
            _motManager.OnSlideCompleted += (sender, slide) =>
            {
                OnSlideShowChanged?.Invoke(this, slide);
            };
        }
                ...
                switch (ci.type)
                {
                    ...
                    default:
                        ...
                        if (MOTAppType != -1 && (ci.type == MOTAppType || ci.type == MOTAppType + 1))
                        {
                            ...
                            {
                                _motManager.HandleMOTDataGroup(_motDecoder.GetMOTDataGroup());
                            }
                            xpadCITypeContinued = MOTAppType + 1;
                        }
                        break;
                }
    ...
    /// <summary>
    /// Alias/subclass for PADDecoder providing both Dynamic Label (DLS) and MOT SlideShow (SLS) decoding.
    /// </summary>
    public class PADDecoder : DynamicLabelDecoder
    {
        public PADDecoder(ILoggingService loggingService) : base(loggingService)
        {
        }
    }
}
